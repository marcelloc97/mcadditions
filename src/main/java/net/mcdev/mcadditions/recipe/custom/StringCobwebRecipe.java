...
public class StringCobwebRecipe extends AbstractCustomRecipe {
	public StringCobwebRecipe(Identifier id, DefaultedList<Ingredient> ingredients, ItemStack output) {
		super(id, ingredients, output);

		MCAdditions.LOGGER.info("Shears Cobweb recipe");
	}

	@Override
	public boolean matches(RecipeInputInventory inventory, World world) {
		boolean foundCobweb = false, foundShears = false;
		int nonEmpty = 0;
		...
		for (int i = 0; i < inventory.size(); i++) {
			...
			nonEmpty++;
			if (stack.isOf(Items.COBWEB)) foundCobweb = true;
			else if (stack.isOf(Items.SHEARS)) foundShears = true;
			else return false;
		}

		return foundCobweb && foundShears && nonEmpty == 2;
	}
